...
@section('title','Cash Collection List')
@section('content')
        ...
        <div class="card-body">
            ...
            <table id="doctable" class="table table-striped table-bordered zero-configuration" cellspacing="0"
              ...
              <tr>
                  ...
                  <td>{{$key->account_head}}</td>
                  <td>{{$key->particular}}</td>
                  <td>{{$key->paymentmode}}</td>
                  <td>{{$key->amount}}</td>
                  <td>
              <a href="{{ url('editexpense/'.$key->id) }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>
              <a class="btn btn-danger btn-sm" href="{{Route('delete_pos_invoices222',$key->id)}}" > <i class="fa fa-trash"></i> </a>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#doctable').DataTable({
            ...
            responsive: true,
            "ajax": {
                "url": " tickets/tickets_load_list?stat=unsolved",
                "type": "POST",
