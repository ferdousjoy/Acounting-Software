            ...
            <div class="card-body">
                    <div class="row">
                        <div class="col-sm-6 cmp-pnl">
                            <div id="customerpanel" class="inner-cmp-pnl">
                                ...
                                <form method="post"" action="{{route('edit_subscription_form')}}">
                                @csrf
                                @method('put')
                                <div id="customer">
                                    <div class="clientinfo">
                                         Client Details                                       
                                          <hr>
                                        <input type="hidden" name="billing_name" value="{{$subscription->billing_name}}">
                                        <div>{{$subscription->billing_name}}</div>
                                    </div>
                                    <div class="clientinfo">
                                    <input type="hidden" name="billing_address" value="{{$subscription->billing_address}}">
                                        <div>  {{$subscription->billing_address}}</div>
                                    </div>
                                    <div class="clientinfo">
                                    <input type="hidden" name="billing_phone" value="{{$subscription->billing_phone}}">
                                        <div>  {{$subscription->billing_phone}}</div>
                                    </div>
                                    <div class="clientinfo">
                                    <input type="hidden" name="billing_email" value="{{$subscription->billing_email}}">
                                        <div>  {{$subscription->billing_email}}</div>
                                    </div>
                                    <hr>   
                                    <p>Warehouse</p>
                        ...
                        <div class="col-sm-6 cmp-pnl">
                            <div class="inner-cmp-pnl">
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-6"><label for="invocieno" class="caption">subscription Number</label>
                                        <div class="input-group">
                                            <div class="input-group-addon"><span class="icon-file-text-o" aria-hidden="true"></span></div>
                                            <input type="text" class="form-control" placeholder="subscription #" name="invocienomber" value="{{$subscription->billing_subscription_no}}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6"><label for="invocieno" class="caption">Reference</label>
                                        <div class="input-group">
                                            <div class="input-group-addon"><span class="icon-bookmark-o" aria-hidden="true"></span></div>
                                            <input type="text" class="form-control" placeholder="Reference #" name="refer" value="{{$subscription->billing_reference}}">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6"><label for="invociedate" class="caption">subscription Date</label>
                                        <div class="input-group">
                                            <div class="input-group-addon"><span class="icon-calendar4" aria-hidden="true"></span></div>
                                            <input type="date" class="form-control " value="{{$subscription->billing_date}}"  placeholder="Billing Date" name="invoicedate"  autocomplete="false" value="{{$subscription->billing_subscription_no}}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6"><label for="invocieduedate"  class="caption">Next Payment After</label>
                                        <select name="nextpayment"  class="selectpicker form-control">
                                            <option selected value="{{$subscription->billing_next_payment}}">{{$subscription->billing_subscription_no}}</option>
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-6">
                                        ...
                                        <select class="form-control round" name="tax"  id="taxformat">
                                            <option value="{{$subscription->billing_tax}}"  selected>&raquo;On</option>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="discount"  id="discountFormat">
                                                <option selected value="{{$subscription->billing_discount}}">{{$subscription->billing_discount}}</option>
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-12">
                                        <label for="toAddInfo"
                                               class="caption">subscription Note</label>
                                        <textarea class="form-control" name="notes" rows="2">{{$subscription->billing_note}}</textarea></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="saman-row">
                        <table class="table-responsive tfr my_stripe">
                            ...
                            <tbody>
                            <tr>
                                <td><input type="text" class="form-control" value="{{$subscription->product_name}}" name="product_name" placeholder="Enter Product name or Code"  id='productname-0'>
                                </td>
                                <td><input type="text" value="{{$subscription->product_quantity}}" class="form-control req amnt" name="product_qty" id="amount-0" onkeypress="return isNumber(event)" onkeyup="rowTotal('0'), billUpyog()" autocomplete="off" value="1"></td>
                                ...
                                <td><input type="text" class="form-control vat " value="{{$subscription->billing__reference}}"  name="product_tax" id="vat-0" onkeypress="return isNumber(event)" onkeyup="rowTotal('0'), billUpyog()" autocomplete="off"></td>
                            ...
                            <tr>
                                ...
                                <textarea id="dpid-0" class="form-control" name="product_description">
                                 {{$subscription->billing_description}}
                                </textarea><br></td>
                            </tr>
                            ...
                            <tr class="sub_c" style="display: table-row;">
                                <td colspan="2">Payment Currency for your client <small>based on live market</small>
                                    <select name="mcurrency" class="selectpicker form-control">
                                        <option selected value="{{$subscription->payment_currency}}">Default</option>
                                ...
                                <td align="left" colspan="2"><input type="text" value="{{$subscription->product_price}}" name="total" class="form-control"  >
                                </td>
                            </tr>
                            <tr class="sub_c" style="display: table-row;">
                                <td colspan="2"> Payment Terms <select name="pterms" class="selectpicker form-control">
                                    <option value="{{$subscription->payment_terms}}">Payment On Receipt</option>
                                    </select></td>
                                <td align="right" colspan="6">
                                <input type="hidden"  value=" {{$subscription->id}}" name="id" >
                                <input type="submit" class="btn btn-success sub-btn" value=" Generate subscription " data-loading-text="Creating...">
                                </td>
                            </tr>
                            </tbody>
                        </table>
